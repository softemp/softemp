        ...
        <div class="col-xs-12">
            ...
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Listar Ocupações</h3>
                    <div class="pull-right">
                        {{--@can('role-store')--}}
                            <a class="btn btn-primary btn-xs"
                               href="{{ route('panel.access.control.occupation.create') }}">Novo</a>
                        {{--@endcan--}}
                    </div>
                </div>
                ...
                <div class="box-body">
                    <table id="table1" class="display responsive nowrap dataTable no-footer dtr-inline collapsed"
                        ...
                        @foreach($data as $occupation)
                            <tr>
                                <td>{{$occupation->role->name}}</td>
                                <td>{{$occupation->name}}</td>
                                {{--@can('role-update','role-view-permissions','role-destroy')--}}
                                <td>
                                    {{--@can('role-update')--}}
                                    <a href="{{ route('panel.access.control.occupation.edit',['id'=>$occupation->id]) }}"
                                       title="Alterar" class="btn btn-xs btn-warning">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    {{--@endcan--}}
{{--                                    @if($occupation->slug != "root" && $occupation->id != env('ROLES_EMPLOYEE_ID'))--}}
                                        {{--@can('role-view-permissions')--}}
                                        {{--<a href="{{ route('panel.access.control.occupation.permissions.edit',['role_id'=>$occupation->id]) }}"--}}
                                        ...
                                        {{--@can('role-destroy')--}}
                                        <a href="#"
                                           onclick="destroy('{{route('panel.access.control.occupation.destroy',['id'=>$occupation->id])}}','{{$occupation->name}}');"
