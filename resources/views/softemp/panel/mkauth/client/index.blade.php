...
@section('content')
    ...
    <div class="box">
        <div class="box-header with-border">
            ...
            <a class="btn btn-success btn-sm" href="{{ route('panel.mkauth.client.active') }}">Clientes Ativos</a>
            @endif
            @if(!Request::is('painel/mkauth/cliente/bloqueado'))
            <a class="btn btn-warning btn-sm" href="{{ route('panel.mkauth.client.blocked') }}">Clientes Bloqueados</a>
            @endif
            @if(!Request::is('painel/mkauth/cliente/desativado'))
            <a class="btn btn-danger btn-sm" href="{{ route('panel.mkauth.client.disabled') }}">Clientes Desativados</a>
            @endif
        ...
        <div class="box-body">
            <table id="table1" class="display responsive nowrap dataTable no-footer dtr-inline collapsed" style="width: 100%;" role="grid">
                ...
                <tr>
                    <td>{{ $client->nome }}</td>
                    <td>
                        {{--<a class="btn btn-primary btn-sm" href="{{ route('panel.mkauth.client.show',['table'=>$client->id]) }}">show</a>--}}
                        <button type="button" class="btn btn-default btn-xs"
                                onclick="showColumns('{{ route('panel.mkauth.client.show',['table'=>$client->id]) }}');">
                            <i class="fa fa-eye"></i>
                        </button>
                    </td>
                </tr>
